...
var Launcher = function(options) {
    ...
    function _parseArgs(names, callback) {
        ...
        if (arguments.length === 1 && _.isFunction(names)) {
            callback = names
            names = getDefaultBrowsersName(null)
        } else {
            names = getDefaultBrowsersName()
    ...
    function getDefaultBrowsersName(browserName) {
    ...
    this.findBrowserById = function(id) {
        var defaultBrowsers = _.keys(options.browsers || options.browsersMapping)
