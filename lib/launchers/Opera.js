...
var OperaBrowser = function() {
    ...
    this._start = function(url) {
        var that = this
        var prefsFile = this._tempDir + this.id +    '/operaprefs.ini'
        fs.writeFile(prefsFile, PREFS, function(err) {
            // TODO(vojta): handle error
            that._execCommand(that._getCommand(), that._getOptions(url))
        })
    }
}

helper.extend(OperaBrowser.prototype, {
    name: 'Opera',
