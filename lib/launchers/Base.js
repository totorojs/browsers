...
var BaseBrowser = function() {
    ...
    this.start = function(url) {
        ...
        var id = this.id = BaseBrowser.generateId()

        try {
            logger.debug('Creating temp dir at ' + this._tempDir + id)
            fs.mkdirSync(this._tempDir + id)
        } catch (e) {}

        this._start(url + '?id=' + id)
    }


    this.is = function(browserName) {
        if (/^\d+$/.test(browserName)) {
            return Object.keys(processList).indexOf(browserName) > -1
        }

        var reg = new RegExp(this.name, 'i')

        if (helper.isString(browserName)) {
    ...
    this.kill = function(callback) {
        ...
        async.forEach(Object.keys(processList), function(subId, cb) {
    ...
    process.on('exit', function() {
        Object.keys(processList).forEach(function(id) {
