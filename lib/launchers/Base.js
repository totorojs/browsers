...
var BaseBrowser = function() {
    ...
    this.start = function(url) {
        ...
        try {
            logger.debug('Creating temp dir at ' + self._tempDir + subId)
    ...
    this.kill = function(callback) {
        logger.debug('Killing ' + self.name)
    ...
    this._onProcessExit = function(code, errorOutput) {
        logger.debug('Process %s exitted with code %d', self.name, code)
        if (code) {
    ...
    process.on('exit', function() {
        ...
        _.keys(pList).forEach(function(id) {
            logger.debug('Cleaning temp dir %s', self._tempDir + id)
