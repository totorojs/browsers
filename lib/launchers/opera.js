...
var OperaBase = function() {
  this.name = 'opera'

  this.start = function() {
    var that = this
    this.createProfile(function(profileDir) {
      that.execCommand(that.getCommand(), that.getOptions(profileDir, that.capture))
    })
  }
  ...
  this.createProfile = function(cb) {
    ...
    if (!fs.existsSync(profileDir)) {
      shelljs.mkdir(profileDir)
    }

    //fs.writeFile(path.join(profileDir, path.sep, 'operaprefs.ini'), PREFS, function(err) {
      cb(profileDir)
    //})
  }

  this.getOptions = function(profileDir, url) {
    // Opera CLI options
    // http://www.opera.com/docs/switches/
    return [url]

    /** Suitable for older browsers(<=12)
    return [
        '-pd', profileDir,
        '-nomail',
        url
